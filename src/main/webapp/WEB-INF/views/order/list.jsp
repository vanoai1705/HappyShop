							...
							<c:forEach var="o" items="${orders}">
								<tr>
									...
									<td><f:formatNumber value="${o.amount}" pattern="#,###" />
										VNĐ</td>
										<c:if test = "${o.status ==1 }"><th>Đang chuẩn bị</td></c:if>
											<c:if test = "${o.status ==2 }"><th>Đang Giao</td></c:if>
											<c:if test = "${o.status ==3 }"><th>Đã Giao</td></c:if>
											<c:if test = "${o.status ==4 }"><th>Đã Huỷ</td></c:if>
									<td><a href="/order/detail/${o.id}"
										class="btn btn-sm btn-warning"> Chi tiết </a></td>
									<td><a href="/order/cancel/${o.id}"
										class="btn btn-sm btn-danger"> Huỷ Đơn </a></td>
								</tr>
							</c:forEach>
