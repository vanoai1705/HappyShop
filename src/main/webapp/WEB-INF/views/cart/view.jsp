	...
	<div class="ct1">
		<h5 style="margin-top:-10px">
			<span style="font-weight: 700;"> <c:choose>
 			...
 			Tổng tiền: <span id="total" style="color: #CC3300"><f:formatNumber
								value="${cart.amount}" pattern="#,###" /> VNĐ</span>
					</c:when>
					<c:otherwise>
 			Tổng tiền: <span id="total" style="color: #CC3300"> 0 VNĐ</span>
					</c:otherwise>
				</c:choose>
						...
						<c:forEach var="p"
							...
							<tr data-id="${p.id}" data-price="${p.unitPrice}"
								...
								<td  style="padding: 50px 0px 40px 0px;"><input id="num"
									value="${p.quantity}" onchange="myChange()" type="number" min="1"
									style="width: 100px; height: 30px; font-size: 16px; border: 1px solid black;">

								</td>
								<td id="amt" class="amt" style="padding: 55px 0px 40px 0px; font-weight: bold; font-size: 16px" >
									<f:formatNumber value="${p.quantity * p.unitPrice * (1 - p.discount)}" pattern="#,###" /> VNĐ
								</td>
								<script>
									function myChange(){
										document.getElementById("amt").innerText= (document.getElementById("num").value*${p.unitPrice * (1 - p.discount)}).toLocaleString()+ " VNĐ";
										document.getElementById("total").innerText= (document.getElementById("num").value*${p.unitPrice * (1 - p.discount)}).toLocaleString()+ " VNĐ";
										console.log("hmm: "+document.getElementById("amt").innerText)
									}
								</script>
