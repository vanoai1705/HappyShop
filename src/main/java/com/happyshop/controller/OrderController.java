...
public class OrderController {
	...
	public String cancelOrder(Model model,@PathVariable("id") Integer id) {
		...
		if(order.getStatus()==1) {
			order.setStatus(4);
			dao.update(order);
			model.addAttribute("message", "Huỷ Đơn Thành Công Mã Đơn #"+order.getId()+"! Cảm Ơn Quý Khách!");
		}
		else {
			model.addAttribute("message", "Không thể huỷ mã đơn #"+order.getId()+ " do trước đó đơn hàng đã hoặc đang được vận chuyển tới khách hàng");
		}
		return "/order/list";
	}
}
